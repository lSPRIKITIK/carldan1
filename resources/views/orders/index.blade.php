...
@section('content')
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <!-- Search Form -->
        <form action="{{ route('orders.index') }}" method="GET" class="flex items-center gap-2 w-full md:w-auto">
            <div class="relative w-80">
                <input type="text" name="search" value="{{ request('search') }}" 
                    placeholder="Search ID or Client Name" 
                    class="border p-2 pr-10 rounded w-full shadow-sm focus:ring-2 focus:ring-blue-300 outline-none">
                
                <!-- Inline Clear Icon (Only shows if search has text) -->
                @if(request('search'))
                    <a href="{{ route('orders.index') }}" class="absolute right-3 top-2.5 text-gray-400 hover:text-red-500 transition">
